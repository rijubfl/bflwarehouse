...
public class WarehouseGRNNewControl {

    private DBConnection dbConnection = new DBConnection();
    private Global objGlobal = Global.getInstance();
    private WarehouseGRNNewGlobal objWarehouseGRNNewGlobal = WarehouseGRNNewGlobal.getInstance();

    private boolean b_Result;
    private ResultSet rs;

    public WarehouseGRNNewControl() {
    ...
    public boolean validateGin(String ginNo) {
        ...
        try {
            ...
            rs = dbConnection.getResultSet("select * from bfldata.dbo.WHGRNDetails where GINNo='" + ginNo + "'", objGlobal.getConnection());
            if (rs.next()) {
            ...
            objGlobal.setErrorMessage("WarehouseGRNNewControl:validateGin:" + ex.toString());
            return false;
        }
    }

    public boolean loadGinDetails(String ginNo) {
        try {
            ...
            if (!dbConnection.insertUpdate("insert into bfldata.dbo.tmpWarehouseGrnScanNew(DeviceId,GINNo,GINDate,WareHouseFrom,WareHouseTo,PalletNo,BoxNo,ToteId,TCount,SCount) " +
                    ...
                    "'" + objWarehouseGRNNewGlobal.getWarehouseTo() + "',palletno,isnull(boxno,''),isnull(ToteID,''),1,0 from usa.dbo.vUPCBoxDet where closed='N' and " +
    ...
    public boolean validateGrn(String ginNo) {
        if (!checkConnection()) {
            return false;
        }
    ...
    public boolean deleteSelectedPallet(String palletno, String boxno) {
        if (!checkConnection()) {
            return false;
        }
        try {
    ...
    public boolean grnSave(String ginNo, String remarks) {
        int sn = 0;
        try {
            ...
            if (!dbConnection.insertUpdate("insert into bfldata.dbo.WHGRNHeader(GRNNo,GRNDate,GRNTime,UserName,Remarks,RecWarehouse) values (" + sn + ",'" + objGlobal.getServerDate() + "'," +
            ...
            if (!dbConnection.insertUpdate("insert into bfldata.dbo.WHGRNDetails select " + sn + ",GINNo,GINDate,PalletNo,WareHouseFrom,WareHouseTo,BoxNo,0,'',ToteId from " +
    ...
    public ArrayList<WarehouseGRNNewScanTicket> loadGinGrnScan() {
        ArrayList<WarehouseGRNNewScanTicket> listGinGrnScan = new ArrayList<WarehouseGRNNewScanTicket>();
        try {
            ...
            while (rs.next()) {
                listGinGrnScan.add(new WarehouseGRNNewScanTicket(rs.getString("PalletNo"), rs.getString("BoxNo"), rs.getString("ToteId")));
            }
    ...
    public ArrayList<WarehouseGRNNewScanCountTicket> loadGinGrnScanCount() {
        ArrayList<WarehouseGRNNewScanCountTicket> listGinGrnScan = new ArrayList<WarehouseGRNNewScanCountTicket>();
        try {
            ...
            while (rs.next()) {
                listGinGrnScan.add(new WarehouseGRNNewScanCountTicket(rs.getString("palletno"), rs.getInt("TCount"), rs.getInt("SCount"), rs.getInt("diff")));
            }
