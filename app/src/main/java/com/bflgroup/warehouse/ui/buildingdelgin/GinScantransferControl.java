...
//                        locCodeTo = rs.getString("locCodeTo");
    ...
    public boolean waySlipsPost(int ginno, String waysSlips, String vehicleType, String vendor) {
        return dbConnection.insertUpdate("Insert into bfldata..PLTIssueWaybill(SrNo,Waybill,VehicleType,Vendor) VALUES('" + ginno + "','" + waysSlips + "','" + vehicleType + "','" + vendor + "')", objGlobal.getConnection());
    }
...
//        }
            ...
            String query2 = "Insert Into bfldata.dbo.PltIssueHead values(" + getGinno + ", '" + DelDate + "', '" + objGlobal.getServerDate() + "', '" + objGlobal.getServerTime() + "', 'ANDR - " + Routeid + "', '" + Driver + "','" + Carno + "','" + remark + "'," + objGlobal.getUserId() + ", '" + Shipno + "', '','" + objGlobal.getWarehouse() + "')";
...
//        }
    ...
    public int getpallets(Context context, String palletno, int routeid) throws SQLException {
        int getCountno = 0;
    ...
    public PalletScanItem getpallet(Context context, String palletno, int prevRouteId) throws SQLException {
        ...
        int routeid = 0;
        PalletScanItem palletScanItem = new PalletScanItem();
        try {
            query2 = "select PalletScan from bfldata..tmpGinRoute where PalletScan = '" + palletno + "' and DeviceName = '" + objGlobal.getDeviceName() + "'";
            ResultSet rs2 = dbConnection.getResultSet(query2, objGlobal.getConnection());
            if (rs2.next()) {
                okMessage("MESSAGE", "Duplicate Pallet", context);
                return new PalletScanItem();
            } else {
                ResultSet rs = dbConnection.getResultSet("select shopname from bfldata..GoodsIssueHead where palletno = '" + palletno + "'",
                        objGlobal.getConnection());
                if (rs.next()) {
                    String shopname = rs.getString("shopname");
                    palletScanItem.setShopname(shopname);
                    String query = "select palletprefix,routeid from bfldata..datasettings where shopname = '" + shopname + "'";
                    ResultSet rs1 = dbConnection.getResultSet(query, objGlobal.getConnection());
                    while (rs1.next()) {
                        arrayList.add(rs1.getString("palletprefix"));
                        ...
                        if (prevRouteId != -1) {
                            if (prevRouteId == routeid)
                                ...
                                palletScanItem.setRouteId(prevRouteId);
                                okMessage("ALERT", "This PalletNo is not in the same route - " + palletno, context);
                    ...
                    if (arrayList.contains(arrOfStr[0])) {
                        ...
                        if (rs5.next()) {
                            getCountno = Integer.valueOf(rs5.getString("count"));
                            palletScanItem.setPalletCount(getCountno);
                            Log.e("getCountno", getCountno + "");
                            ObjGinScanTransferShared.savePalletno(palletno);
                            ObjGinScanTransferShared.savePalletCount(String.valueOf(getCountno));
                            ObjGinScanTransferShared.saveRouteid(String.valueOf(routeid));
                    ...
                    vibrate(1000, context);
                    okMessage("Alert", "Invalid Pallet", context);
                }
            }
        } catch (Exception e) {
            vibrate(1000, context);
