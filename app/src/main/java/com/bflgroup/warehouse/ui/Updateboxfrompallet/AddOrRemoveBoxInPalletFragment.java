package com.bflgroup.warehouse.ui.Updateboxfrompallet;
...
public class AddOrRemoveBoxInPalletFragment extends Fragment implements View.OnClickListener {
    ...
    BoxInAndOutControl boxInAndOutControl = new BoxInAndOutControl();
    ...
    private BoxInOutAdapter boxInOutAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rootView = inflater.inflate(R.layout.fragment_add_or_remove_box_in_pallet, container, false);
        initViews();
        setOnclickListeners();
        setViews();
        return rootView;
    ...
    private void setViews() {
        boxInOutDataList = boxInAndOutControl.checkTempData();
        if (boxInOutDataList != null && !boxInOutDataList.isEmpty()) {
            ...
            newBoxInOutDataList = boxInAndOutControl.checkNewTempData();
...
//                }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_scan:
                if (validateFields()) {
                    boxInOutDataList = boxInAndOutControl.palletsDetails(etPalletNo.getText().toString());
                    boxInOutAdapter = new BoxInOutAdapter(boxInOutDataList);
                    lvBoxInOut.setAdapter(boxInOutAdapter);
                    totalQty = 0;
                    for (int i = 0; i < boxInOutDataList.size(); i++) {
...
//                                + " to the pallet" + etPalletNo.getText().toString() + " ?", 1);
                            ...
                            boolean dataStatus = boxInAndOutControl.checkTempDataForDelete(etBoxNo.getText().toString());
                            if (dataStatus) {
...
//                            showAlert("Error!!", "Box " + etBoxNo.getText().toString() + " is not in the pallet " + etPalletNo.getText().toString(), 0);
    ...
    private class BoxInOutAdapter extends BaseAdapter {
        public List<BoxInOutRequestData> boxInOutList;

        public BoxInOutAdapter(List<BoxInOutRequestData> boxInOutList) {
    ...
    private boolean validBoxFields() {
        ...
        if (etBoxNo.getText().toString().equals("")) {
            ...
            vibrate(500);
        } else if (boxInAndOutControl.boxCheckInTempDB(etBoxNo.getText().toString())) {
            if (rbAdd.isChecked()) {
                status = false;
        ...
        } else if (!boxInAndOutControl.boxChecking(etBoxNo.getText().toString())) {
            status = false;
            ...
            if (!boxInAndOutControl.addBoxChecking(boxInOutDataList.get(0).selectedPalletNo, etBoxNo.getText().toString())) {
                status = false;
            ...
            public void onClick(DialogInterface dialog, int id) {
                if (flag == 1) {
                    ...
                    for (int i = 0; i < newBoxInOutDataList.size(); i++) {
                        ...
                        int sn = boxInAndOutControl.palletChecking(newBoxInOutDataList.get(i).selectedPalletNo);
                        //    boxInAndOutControl.updateSn(boxInOutDataList.get(0).boxNo, sn);
                        boolean status;
                        if (newBoxInOutDataList.get(i).type == 1) {
                            status = boxInAndOutControl.saveAddedBoxToDB(newBoxInOutDataList.get(i).selectedBoxNo, newBoxInOutDataList.get(i).selectedPalletNo);
                            if (status)
...
//                        }
                            ...
                            boxInAndOutControl.updateSn(newBoxInOutDataList.get(i).boxNo, sn);
                            status = boxInAndOutControl.updatePallets(newBoxInOutDataList.get(i).boxNo, newBoxInOutDataList.get(i).selectedPalletNo, "REMOVE");
                            if (status)
